...
def build_pdf(snpedia_csv, trait_csv, eq_csv, user_display_name="",
    ...
    # Show-everything behavior: no magnitude floor and no note filter, so every
    # annotated variant appears (including blank-note rows), matching the
    # data-rich report. Topic grouping and the per-article cap still apply to
    # keep individual tables readable.
